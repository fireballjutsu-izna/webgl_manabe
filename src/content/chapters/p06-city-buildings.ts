...
[2-11](#/ch/t11-performance)でやったとおり、**動かないものは合体させます**（\`mergeGeometries\`）。
これで 1 回になります。ところが、合体すると困ることが 1 つ出ます。

**マテリアルが 1 つになるので、建物ごとに色を変えられません。**

解決は{{attribute}}です。[2-12](#/ch/t12-shader-intro)でやったとおり、
...
そこで、[2-04](#/ch/t04-texture)でやった UV を**自分で書き換えます。**
...
[2-11](#/ch/t11-performance)で「まずこれを画面に出してください」と書いたとおり、
...
カリングのほうは 3-08 で**分割して合体する**ことで取り戻します。`,
    },
  ],
  quiz: [
    {
