...
道路の網を解析する必要はありません。[3-05](#/ch/p05-city-layout)の作り方を思い出してください。
...
角を丸めたいので、[1-12](#/ch/12-curve)でやった \`CatmullRomCurve3\` に
...
`,
    },
    {
      ...
      worked: {
        ...
        result: '**経路の長さで割ることで、どの街区でも「毎秒 4」で走ります。** 割らずに `u += 0.0833` と固定で書くと、どの経路も 12 秒で一周してしまい、**大きい街区を回る車ほど速くなります**。1-12 の `getPointAt` と同じ「道のりで測る」考え方です。',
      },
    },
    {
      kind: 'callout',
...
[3-06](#/ch/p06-city-buildings)で、街全体を 1 つに合体させると
...
- **建物に窓の凹凸を付ける。** 3-02 のバンプマップを facade に貼る
- **ポストプロセス**（\`EffectComposer\`）。夜景にブルームを乗せると化けます
- **物理エンジン**（Rapier など）。落ちる・当たる・積み上がる
- **地形**。3-02 のノイズを平面の頂点の高さに使えば、そのまま山になります
...
街区の大小にかかわらず**同じ速さ**で走ります。1-12 の \`getPointAt\` と同じ、「道のりで測る」考え方です。`,
    },
  ],
  quiz: [
    {
