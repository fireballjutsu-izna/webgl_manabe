...
`,
      caption:
        '放っておくと 80 秒で 1 日がめぐります。スライダーを動かすと自動送りが止まります。**カメラを回してみてください。ドローコールの数字が動きます** ― 視界の外の区画が省かれている証拠です。街全体を 1 つに合体させていたら、この数字は動きません。なお衝突判定はしていないので、交差点で車どうしがすれ違います。 **コードの地図で色が付いているところが、この章で足したぶんです。** 残りは[](#/ch/p05-city-layout)から[](#/ch/p07-city-light)までで作ったもので、下ごしらえは末尾にまとめてあります。',
    },
    {
      kind: 'md',
      text: `
