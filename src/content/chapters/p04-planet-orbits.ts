...
新しい概念はほとんどありません。[1-09 座標空間の階層](#/ch/09-hierarchy)、
[1-05 三角関数](#/ch/05-trig)、[1-08 補間](#/ch/08-interp)、[2-08 Raycaster](#/ch/t08-raycaster)を
...
[1-07](#/ch/07-rotation)で見たとおり、順番の都合で軸が寝てしまいます。
...
これは[1-09](#/ch/09-hierarchy)でやった「親の座標系の中で子が動く」そのものです。
`,
    },
    {
      kind: 'callout',
...
`,
    },
    {
      ...
      readAloud:
        'R が軌道の半径、ω（オメガ）が回る速さ、φ（ファイ）が最初の位置のずれ（位相）です。t が時間。1-05 の単位円をそのまま大きくして、時間で角度を進めているだけです。φ を変えれば、同じ軌道の別の場所から回りはじめます。',
      worked: {
...
[2-08](#/ch/t08-raycaster)でやった \`Raycaster\` で「クリックの先に何があるか」を調べ、
見つかった相手へ**カメラをなめらかに動かします。**

なめらかに動かす部分は、[1-08 補間](#/ch/08-interp)の \`lerp\` そのものです。
毎フレーム、**いまの位置から目標へ少しだけ近づける**。これだけで自然な減速が付きます。

このあとのコードが**完成品**です。3-02 の地表、3-03 の雲・大気・夜の明かり、
...
`,
    },
    {
      ...
      worked: {
        ...
        result: '**0.3 → 0.285 → 0.271** と、**勝手に減速していきます**。イージングの関数を 1 つも書いていないのに、「近づくほどゆっくり」が出るのがこの式の気持ちよさです。ただし 1-08 で見たとおり、これは**フレームレートに依存する**ので、厳密にやるなら $k$ を `dt` から作り直します。',
      },
    },
    {
      kind: 'callout',
...
厳密に揃えたいなら、[2-06](#/ch/t06-loop-clock)でやったとおり時間を使います。
...
と読めば[1-09](#/ch/09-hierarchy)の話そのものです。
...
- **3-01** … 骨組みと星空。球面に一様にばらまく方法
- **3-02** … 地表をノイズで描く。方向でノイズを引いて継ぎ目を消す
- **3-03** … 大気・雲・夜の明かり。内積で縁と昼夜を切り出す
- **3-04** … 傾いた自転、公転、クリックで寄る視点、ラベル
...
[2-11 速くする](#/ch/t11-performance)でやったドローコールの話が、そこで本番を迎えます。
`,
    },
  ],
  exercises: [
