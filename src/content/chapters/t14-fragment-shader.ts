...
シェーダの $3$ 章はこれで終わりです。振り返ると、やってきたのはこういうことでした。
...
**土台の数学が、どの場面でも顔を出していた**はずです。
ベクトルと内積が分かっていれば、シェーダまで地続きでした。

**残りは $3$ 章です。** [](#/ch/w48-shader-debug)でシェーダが動かないときの直し方を、
[](#/ch/w49-onbeforecompile)で「ライトを効かせたまま頂点だけ動かす」手を覚え、
[](#/ch/w50-small-forest)で全部をまとめて $1$ つの作品にします。
`,
    },
    {
      kind: 'callout',
