...
なかでも **UV**（[2-04 テクスチャ](#/ch/t04-texture)）が主役になります。
...
ここが第1部の総決算です。[1-03 内積](#/ch/03-dot)と
[1-11 法線とライティング](#/ch/11-normal-light)でやったことを、そのまま GLSL に書きます。
...
`,
    },
    {
      ...
      worked: {
        ...
        result: '1-11 で手で計算したのと、**一字一句同じ結果**です。違うのは、そこでは 1 点だったのが、ここでは**画素ごとに GPU が同時にやっている**という点だけ。数学は 1 つも増えていません。',
      },
    },
    {
      kind: 'md',
...
（[1-02 ベクトル](#/ch/02-vector)の正規化の話です）。
...
1-05 の「波の細かさを上げると格子が崩れる」とまったく同じ現象で、
