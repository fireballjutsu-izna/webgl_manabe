...
テクスチャを読み込むときには関係してきます（[](#/ch/t04-texture)で扱います）。
`,
    },
    {
      kind: 'callout',
...
`,
  ...
  exercises: [
    {
      prompt: 'サンドボックスの一番上の行（\`metalness\` が 1 の行）が、どの粗さでもほとんど暗いままなのはなぜでしょう。どうすれば金属らしくなりますか。',
      hint: '鏡を真っ暗な部屋に置いたら、何が映りますか。',
      answer: `**映り込むものが何も無いから**です。金属は自分の色をほとんど返さず、まわりの景色を映して見せる素材なので、
まわりが真っ暗なら真っ暗に映ります。ライトを増やしても、映るのは点の反射だけで金属には見えません。
必要なのは \`scene.environment\`（環境マップ）です。**その作り方は [](#/ch/q01-environment) でやります。**`,
    },
    {
      prompt: '\`MeshStandardMaterial\` を \`MeshBasicMaterial\` に置き換えてください（\`roughness\` と \`metalness\` は無視されます）。見た目はどうなりますか。どんなときに使いますか。',
      hint: 'このマテリアルは、ライトの影響を受けません。',
      answer: `陰影が完全に消え、**のっぺりした単色の円**になります。光を一切計算しないマテリアルだからです。
ふつうの物体には向きませんが、**ライティングに左右されたくないもの**にはこれが正解です。
このサイトのデモで、ベクトルの矢印がどの角度でも同じ色に見えるのは \`MeshBasicMaterial\` を使っているからで、
そうしないと「シアン＝入力 a」という約束が向きによって崩れてしまいます。`,
    },
  ],
  quiz: [
    {
