...
[](#/ch/11-normal-light)で見た反射ベクトルが、そのまま効いています。
...
`,
    },
    {
      ...
      readAloud:
        '視線 v が法線 n の面で跳ね返る向き r を出す式です。法線とライティングの章でやった反射ベクトルそのもの。金属はこの r の方向にあるものを映します。環境マップは「あらゆる方向に何があるか」を引ける表なので、r を渡すだけで色が返ってきます。',
      worked: {
    ...
    },
    {
      ...
      id: 'envmap-compare',
      caption:
        '「映り込ませるもの」を「なし」にした瞬間、金属もガラスも黒く沈みます。これが [](#/ch/t03-material) で言っていた状態です。粗さを上げていくと映り込みがぼやけ、金属らしさが「色」ではなく「まわりが映っていること」から来ているのが分かります。ガラスは背景を消すと真っ黒になります ― 透かす先が無いからです。',
    },
    {
      kind: 'callout',
      tone: 'analogy',
...
上が空色・下が地面色のグラデーションを[](#/ch/t14-fragment-shader)のシェーダで塗り、
...
空の色を時間で変えたい（[](#/ch/p07-city-light)のような昼夜）場合でも、
