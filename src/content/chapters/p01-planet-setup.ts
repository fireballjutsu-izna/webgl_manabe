...
- **[3-02](#/ch/p02-planet-surface)** … 惑星の表面をコードで描く（海・陸・雪・凹凸）
- **[3-03](#/ch/p03-planet-atmosphere)** … 大気の光、雲、昼と夜の境目
- **[3-04](#/ch/p04-planet-orbits)** … 自転と公転、月、クリックで寄っていく視点
...
最初のコードに新しいものはほとんどありません。[2-01](#/ch/t01-first-scene) の 3 点セット、
[2-06](#/ch/t06-loop-clock) の {{描画ループ}}、[2-07](#/ch/t07-controls) の \`OrbitControls\` を並べるだけです。
...
[1-02](#/ch/02-vector) でやった{{正規化}}です。これは**立方体の角の方向に寄ります。**
...
`,
    ...
    },
    {
      ...
      readAloud:
        'x は r かけるコサインシータ、y はそのまま y、z は r かけるサインシータ。1-05 でやった「角度から座標を出す」を、切り口の円の上でやっているだけです。',
      worked: {
        ...
        result: '$(0.4,\\; 0.6,\\; 0.693)$。**1-05 の単位円を、高さ $y$ の切り口の上でやっているだけ**です。半径が 1 ではなく $r$ になっているところだけが違います。',
      },
    },
    {
      kind: 'callout',
...
[1-10 カメラと投影](#/ch/10-camera)で見たとおり、near を小さくしすぎるのは
...
2-07 の \`maxPolarAngle\` で床下に潜れないようにしたのと、まったく同じ考え方です。
