...
Three.js でその「三角形の集まり」を持っているのが\`BufferGeometry\`で、
\`BoxGeometry\` や \`SphereGeometry\` はその作り方を用意してくれた便利な子クラスにすぎません。

**ジオメトリは形だけを持ちます。**色や質感はマテリアル（次の章）の担当で、
置き場所や向きは \`Mesh\` が持ちます。この 3 つの分担がはっきりしていることが、
...
これがインデックスです。立方体なら 36 頂点が 8 頂点に、球ならもっと劇的に減ります。
...
`,
    },
    {
      ...
      text: `
自分で組んだジオメトリには法線が入っていません。[1-11 法線とライティング](#/ch/11-normal-light)
でやったとおり、明るさは法線と光の内積で決まるので、法線がなければ計算のしようがありません。
\`geometry.computeVertexNormals()\` を呼ぶと、[1-04 外積](#/ch/04-cross)の要領で
2 辺の外積から自動的に作ってくれます。
`,
    },
    {
...
- **uv** … テクスチャのどこを貼るか（[2-04 テクスチャ](#/ch/t04-texture)で扱います）

ほかに \`color\` を持たせて頂点ごとに色を変えることもできますし、
自分で決めた名前の属性を追加して、シェーダから読むこともできます
（[2-12 シェーダの入口](#/ch/t12-shader-intro)）。
`,
    },
    {
      kind: 'callout',
